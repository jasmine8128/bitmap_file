...
file_ob=open("output.txt",'w')

if __name__ == '__main__':
	for i in xrange(input_length):
		print input_data[i], row_id[i], chunk_id[i], bin(literal[i])
		file_ob.writelines(str(input_data[i])+' '+str(row_id[i])+' '+str(chunk_id[i])+' '+str(bin(literal[i]))+'\n')
	print len(scanned_input_data)
	file_ob.write(str(len(scanned_input_data))+'\n')
	for i in xrange(len(scanned_input_data)):
		print scanned_input_data[i], scanned_chunk_id[i], bin(scanned_literal[i]), fill_0_word[i]
		file_ob.writelines(str(scanned_input_data[i])+' '+str(scanned_chunk_id[i])+' '+str(bin(scanned_literal[i]))+' '+str(fill_0_word[i])+'\n')
	print len(out_index)
	file_ob.write(str(len(out_index))+'\n')
	for i in out_index:
		print bin(i)
		file_ob.write(bin(i)+'\n')
